...
set entity_names_on [ are_entity_names_on ]
	...
	set DQSpathjitter [expr [get_io_standard_node_delay -dst DQDQS_JITTER -io_standard $io_standard -parameters [list IO $::GLOBAL_ddr_p0_io_interface_type] -in_fitter]/1000.0]
	
	set DQSpathjitter_setup_prop [expr [get_io_standard_node_delay -dst DQDQS_JITTER_DIVISION -io_standard $io_standard -parameters [list IO $::GLOBAL_ddr_p0_io_interface_type] -in_fitter]/100.0]

	
	set tJITper [expr [get_io_standard_node_delay -dst MEM_CK_PERIOD_JITTER -io_standard $io_standard  -parameters [list IO $::GLOBAL_ddr_p0_io_interface_type] -in_fitter -period $t(CK)]/2000.0]
...
foreach { inst } $instances {
	...
	set fpga(tDQS_PHASE_JITTER) [ expr [ get_integer_node_delay -integer $::GLOBAL_ddr_p0_dqs_delay_chain_length -parameters {IO MAX HIGH} -src DQS_PHASE_JITTER -in_fitter ] / 1000.0 ]
	...
	if { $debug } {
		...
		post_message -type info "SDC: -----------------"
		post_message -type info "SDC:    DQS Phase: $::GLOBAL_ddr_p0_dqs_delay_chain_length"
	...
	# PLL Clock Names
	set local_pll_afi_clk "${inst}|ddr_p0_pll_afi_clk"
	set local_pll_mem_clk "${inst}|ddr_p0_pll_mem_clk"
	set local_pll_addr_cmd_clk "${inst}|ddr_p0_pll_addr_cmd_clk"
	set local_pll_write_clk "${inst}|ddr_p0_pll_write_clk"
	set local_pll_afi_half_clk "${inst}|ddr_p0_pll_afi_half_clk"
	set local_pll_avl_clock "${inst}|ddr_p0_pll_avl_clock"
	set local_pll_config_clock "${inst}|ddr_p0_pll_config_clock"
	...
	# W A R N I N G !
	# The PLL parameters are statically defined in the ddr_p0_parameters.tcl
	...
	# apply those changes to the PLL parameters in the ddr_p0_parameters.tcl

	set pll_i 5
	create_generated_clock -add -name "$local_pll_afi_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult(0) -divide_by $::GLOBAL_ddr_p0_pll_div(0) -phase $::GLOBAL_ddr_p0_pll_phase(0) $pll_afi_clock
	if {[string compare -nocase $pll_afi_clock $pll_ck_clock]==0} {
		set local_pll_mem_clk $local_pll_afi_clk
	} else {
		create_generated_clock -add -name "$local_pll_mem_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult(1) -divide_by $::GLOBAL_ddr_p0_pll_div(1) -phase $::GLOBAL_ddr_p0_pll_phase(1) $pll_ck_clock
	}
	create_generated_clock -add -name "$local_pll_addr_cmd_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult(3) -divide_by $::GLOBAL_ddr_p0_pll_div(3) -phase $::GLOBAL_ddr_p0_pll_phase(3) $pll_ac_clock
	set write_clk_phase $::GLOBAL_ddr_p0_pll_phase(2)
	create_generated_clock -add -name "$local_pll_write_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult(2) -divide_by $::GLOBAL_ddr_p0_pll_div(2) -phase $write_clk_phase $pll_write_clock
	create_generated_clock -add -name "$local_pll_afi_half_clk" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult(4) -divide_by $::GLOBAL_ddr_p0_pll_div(4) -phase $::GLOBAL_ddr_p0_pll_phase(4) $pll_afi_half_clock
	set avl_pll_clk_i $pll_i
	if {[string compare -nocase $pll_afi_clock $pll_avl_clock]==0} {
		set local_pll_avl_clock $local_pll_afi_clk
	} else {
		create_generated_clock -add -name "$local_pll_avl_clock" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult($pll_i) -divide_by $::GLOBAL_ddr_p0_pll_div($pll_i) -phase $::GLOBAL_ddr_p0_pll_phase($pll_i) $pll_avl_clock
	}
	incr pll_i
	create_generated_clock -add -name "$local_pll_config_clock" -source $pll_ref_clock -multiply_by $::GLOBAL_ddr_p0_pll_mult($pll_i) -divide_by $::GLOBAL_ddr_p0_pll_div($pll_i) -phase $::GLOBAL_ddr_p0_pll_phase($pll_i) $pll_config_clock
	incr pll_i
	...
	create_generated_clock -add -name "${inst}|memphy_leveling_clk" -master_clock [get_clocks $local_pll_write_clk] -source $pll_write_clock -phase $::GLOBAL_ddr_p0_leveling_capture_phase [ get_pins $leveling_pins ]
	...
	if {(270 - $::GLOBAL_ddr_p0_pll_phase(3)) > 90 || ($::GLOBAL_ddr_p0_pll_phase(3) - 270) > 90} {
		foreach { ck_pin } $ck_pins {
	...
	if {(($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
		set dqs_clocks [ddr_p0_get_all_instances_dqs_pins ddr_p0_ddr_db]
