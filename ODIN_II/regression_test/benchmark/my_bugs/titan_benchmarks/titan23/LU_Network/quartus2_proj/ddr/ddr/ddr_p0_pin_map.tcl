...
proc ddr_p0_get_ddr_pins { instname allpins } {
	...
	if {$pll_afi_clock_id == -1} {
		...
		post_message -type critical_warning "ddr_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_ck_pins)]"
	} else {
	...
	if {$pll_afi_half_clock_id == -1} {
		...
		post_message -type critical_warning "ddr_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_half_ck_pins)]"
	} else {
	...
	if {$pll_avl_clock_id == -1} {
		...
		post_message -type critical_warning "ddr_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_ck_pins)]"
	} else {
	...
	if {$pll_config_clock_id == -1} {
		...
		post_message -type critical_warning "ddr_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(config_ck_pins)]"
	} else {
	...
	if {$pll_ref_clock_id == -1} {
		post_message -type critical_warning "ddr_p0_pin_map.tcl: Failed to find PLL reference clock"
	} else {
...
proc ddr_p0_initialize_ddr_db { ddr_db_par } {
	...
	set instance_list [get_core_instance_list $::GLOBAL_ddr_p0_corename]
