...
source "$script_dir/DE4_SOPC_ddr2_0_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	set clk_to_write_d [DE4_SOPC_ddr2_0_p0_traverse_to_ddio_out_pll_clock [lindex $all_dq_pins 0] msg_list]
	...
	# DQS_phase offset
	set dqs_phase [ DE4_SOPC_ddr2_0_p0_get_dqs_phase $dqs_pins ]

	# Get the interface type (HPAD or VPAD)
	set interface_type [DE4_SOPC_ddr2_0_p0_get_io_interface_type $all_dq_pins]
	...
	set io_std [DE4_SOPC_ddr2_0_p0_get_io_standard [lindex $dqs_pins 0]]
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		set model_corner [DE4_SOPC_ddr2_0_p0_get_model_corner]
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename}-$inst)
			if {!$assumptions_valid} {
				...
				set assumptions_valid [DE4_SOPC_ddr2_0_p0_verify_flexible_timing_assumptions $inst pins $mem_if_memtype]
			set assumptions_cache(${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename}-$inst) $assumptions_valid
		}	
		...
		# Write Analysis
			...
			DE4_SOPC_ddr2_0_p0_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family $DQS_min_max $interface_type $max_package_skew $dll_length $period pins t summary MP IP board

		#######################################
		# Read Analysis

			DE4_SOPC_ddr2_0_p0_perform_flexible_read_capture_timing_analysis $opcs $opcname $inst $family $DQS_min_max $io_std $interface_type $max_package_skew $dqs_phase $period $all_dq_pins pins t summary MP IP board fpga
		DE4_SOPC_ddr2_0_p0_perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename} $family $DQS_min_max $io_std $interface_type $period pins t summary MP IP board fpga SSN

		#######################################
		# PHY and Address/command Analyses

		DE4_SOPC_ddr2_0_p0_perform_ac_analyses  $opcs $opcname $inst $DQS_min_max pins t summary IP		
		DE4_SOPC_ddr2_0_p0_perform_phy_analyses $opcs $opcname $inst $inst_controller pins t summary IP
		

		#######################################
		# Bus Turnaround Time Analysis
			DE4_SOPC_ddr2_0_p0_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
	...
	set summary [lsort -command DE4_SOPC_ddr2_0_p0_sort_proc $summary]

	set f -1
	if { [DE4_SOPC_ddr2_0_p0_get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename} - Instance: $inst"
	...
	post_message -type info "Core: ${::GLOBAL_DE4_SOPC_ddr2_0_p0_corename} - Instance: $inst"
	...
	foreach summary_line $summary {
		...
		if {$su != "--"} {
			set su [ DE4_SOPC_ddr2_0_p0_round_3dp $su]
		}
		if {$hold != "--"} {
			set hold [ DE4_SOPC_ddr2_0_p0_round_3dp $hold]
		}
